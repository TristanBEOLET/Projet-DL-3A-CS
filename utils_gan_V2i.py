...
def train_model(
    ...
    for epoch in range(hyperparameters_gn.num_epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            dice_loss = 1 - metrics.dice(outputs > hyperparameters_gn.treshold, labels) #essayer sans tresold
            mse_loss = criterion_gn(outputs, labels)
            loss_gene = dice_loss + mse_loss + (1-loss_discri)#rajouter loss est ce que j'ai trompé le discri ? 
            loss_gene.backward()
            optimizer_gn.step()
